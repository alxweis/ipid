...
restartProbing:
	createRecvChan(target, 3-attempts)
...
// Receive Channel
func createRecvChan(target string, retryAttempt int) {
	createProbe(target)
	_, ok := getRecvChan(target)
	if ok {
		log.Printf("Receive Channel [%s] already exists (retry_attempt=%d)", target, retryAttempt)
	}
