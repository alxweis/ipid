#!/bin/bash

OUTPUT_FILE="$1"

count_lines() {
    wc -l < "$1"
}

INITIAL_COUNT=$(count_lines "$OUTPUT_FILE")

# Deduplicate
echo "Start duplicating..."
START_TIME_DEDUP=$(date +%s)

TEMP_FILE=$(mktemp "${OUTPUT_FILE}.dedup.XXXXXX" -p .)
LC_ALL=C sort -t',' -k1,1 -u -T . "$OUTPUT_FILE" > "$TEMP_FILE"
mv "$TEMP_FILE" "$OUTPUT_FILE"

END_TIME_DEDUP=$(date +%s)
RUNTIME_DEDUP=$((END_TIME_DEDUP - START_TIME_DEDUP))
UNIQUE_IPS=$(count_lines "$OUTPUT_FILE")
REMOVED_IPS=$((INITIAL_COUNT - UNIQUE_IPS))
echo "Deduplication finished: removed_ips=$REMOVED_IPS runtime=$RUNTIME_DEDUP seconds"

# Shuffle
echo "Start shuffle..."
START_TIME_SHUFFLE=$(date +%s)

TEMP_FILE_RND_IDX=$(mktemp "${OUTPUT_FILE}.rnd_idx.XXXXXX" -p .)
awk -v OFS=',' '{print int(rand()*1e18), $0}' "$OUTPUT_FILE" > "$TEMP_FILE_RND_IDX"
TEMP_FILE_SORTED_RND_IDX=$(mktemp "${OUTPUT_FILE}.sorted_rnd_idx.XXXXXX" -p .)
LC_ALL=C sort -t',' -k1,1n -T . "$TEMP_FILE_RND_IDX" > "$TEMP_FILE_SORTED_RND_IDX"
rm "$TEMP_FILE_RND_IDX"
TEMP_FILE_SHUFFLED=$(mktemp "${OUTPUT_FILE}.shuffled.XXXXXX" -p .)
cut -d',' -f2- "$TEMP_FILE_SORTED_RND_IDX" > "$TEMP_FILE_SHUFFLED"
rm "$TEMP_FILE_SORTED_RND_IDX"
mv "$TEMP_FILE_SHUFFLED" "$OUTPUT_FILE"

END_TIME_SHUFFLE=$(date +%s)
RUNTIME_SHUFFLE=$((END_TIME_SHUFFLE - START_TIME_SHUFFLE))
echo "Shuffle finished: runtime=$RUNTIME_SHUFFLE seconds"

final_count=$(count_lines "$OUTPUT_FILE")
echo "IP Scan finished: final_count=$final_count"

# Add header
echo "Adding header..."
sed -i '1i IP,TS' "$OUTPUT_FILE" || { echo "Failed to add header"; exit 1; }

# Compress the file
echo "Compressing file..."
OUTPUT_COMPRESSED_FILE="${OUTPUT_FILE}.zst"
zstd -f "$OUTPUT_FILE" -o "$OUTPUT_COMPRESSED_FILE" || { echo "Failed to compress file"; exit 1; }

if [ $? -eq 0 ]; then
  rm "$OUTPUT_FILE"
fi

OUTPUT_FILE=$OUTPUT_COMPRESSED_FILE
echo "Results saved in $OUTPUT_FILE"
